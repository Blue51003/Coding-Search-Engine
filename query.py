...
def load_documents():
    ...
    for line in document_lines:
        document = ast.literal_eval(line.strip())  # Use ast.literal_eval to parse the list from string
...
def get_tf_dictionary(term):
    ...
    for document in tf_values:
        tf_values[document] /= len(documents[int(document)])+1
...
def calculate_sorted_order_of_documents(query_terms):
    ...
    for term in query_terms:
        ...
        for document_index in potential_documents:
            print('Document: ', documents[int(document_index)-1], ' Score: ', potential_documents[document_index])
